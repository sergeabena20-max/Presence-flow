...
async function authorize(id: string) {
  ...
  const admin = await prisma.user.findFirst({
    where: { id, role: "ADMIN", ...(session.role === "ADMIN" ? { organizationId: session.organizationId! } : {}) },
...
export async function PATCH(request: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  try {
    ...
    if (auth.admin.id === auth.session.userId && data.isActive === false) return NextResponse.json({ error: "Vous ne pouvez pas désactiver votre propre compte administrateur." }, { status: 400 })
    const admin = await prisma.user.update({ where: { id }, data, select: { id: true, organizationId: true, firstName: true, lastName: true, email: true, phone: true, isActive: true, createdAt: true } })
