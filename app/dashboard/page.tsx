...
function QuickActions({ isSuperAdmin }: { isSuperAdmin: boolean }) { return <section className="dashboard-actions"><div className="section-heading"><div><p className="dashboard-eyebrow">GESTION</p><h2>Accès rapides</h2></div></div><div className="action-grid">
  {isSuperAdmin && <a className="action-card" href="/dashboard/organisations"><span className="action-icon">⌘</span><div><strong>Organisations</strong><p>Créer, superviser et supprimer les organisations.</p></div><span>→</span></a>}
  ...
  {!isSuperAdmin && <a className="action-card" href="/dashboard/administrateurs"><span className="action-icon">♙</span><div><strong>Administrateurs</strong><p>Gérer les comptes administrateurs.</p></div><span>→</span></a>}
  {!isSuperAdmin && <a className="action-card" href="/dashboard/personnel"><span className="action-icon">♙</span><div><strong>Personnel</strong><p>Gérer les utilisateurs inscrits.</p></div><span>→</span></a>}
  ...
  {!isSuperAdmin && <a className="action-card" href="/dashboard/parametres"><span className="action-icon">⚙</span><div><strong>Paramètres</strong><p>Configurer le GPS et les horaires.</p></div><span>→</span></a>}
</div></section> }
